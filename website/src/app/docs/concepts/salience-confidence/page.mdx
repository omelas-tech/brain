...
export const metadata = docMeta("/docs/concepts/salience-confidence", {
...
A third, related dimension — **origin**, *where the content came from* — caps what values both of these scores may take and weighs into recall ranking. See [Provenance & Trust](/docs/concepts/provenance-trust/).
...
Memories with salience **>= 0.7** are **never auto-pruned**. Even if their strength decays below the archive threshold (0.1), they will not be moved to `_archived/` during the [sleep cycle](/docs/advanced/sleep-cycle/). They must be explicitly removed via `/brain:forget` (or `/brain:forget --deep` for full forensic erasure).
...
During [consolidation](/docs/commands/sleep/) (a phase of the sleep cycle), the memory with the highest salience in a group serves as the **anchor**. Its framing, key details, and perspective take priority in the synthesized result. Lower-salience memories in the group contribute supplementary details.
