...
export const metadata = docMeta("/docs", {
...
Brain Memory works with **Claude Code**, **OpenAI Codex CLI**, **OpenCode**, **GitHub Copilot CLI**, **Kilo**, and **Google Antigravity** — with the **Claude.ai and ChatGPT apps** through a hosted [MCP](https://modelcontextprotocol.io) connector, and with **OpenClaw** and **Hermes Agent** through [native integrations](https://github.com/omelas-tech/brain/tree/main/integrations). Claude Code and Codex can also install it as a [plugin](/docs/getting-started/installation/#plugin-install) from the `omelas-tech/brain` marketplace, which adds deterministic session hooks.
