...
export const metadata = docMeta("/docs/commands/forget", {
...
## Standard forget vs. `--deep` (forensic erasure)

By default `/brain:forget` handles a single memory. If you need to erase all traces of a topic — including references in other memories, association links, and index entries — add the `--deep` flag for **forensic erasure**.

| Feature | `/brain:forget` | `/brain:forget --deep` |
|---|---|---|
...
| Use case | Outdated single memory | Complete topic erasure (e.g. leaked secrets) |
