...
export const metadata = docMeta("/docs/commands/sync", {
...
The easiest way to sync — no Git setup required. Push your brain to [Brain Cloud](https://app.brainmemory.ai) and pull it on any device.

### Setup

<Steps>
  <Step title="Create an account">
    Sign up at [app.brainmemory.ai](https://app.brainmemory.ai/register).
  </Step>
...
Brain Cloud is also reachable via the `brain cloud` CLI for use outside of agent sessions:
...
A push names the archive it started from. If another device pushed since you last pulled, the store refuses the upload and nothing is overwritten:

```
Error: The store has changes you have not pulled, so nothing was uploaded.
```

Run `brain cloud pull`, then `brain cloud push` again. `brain cloud push --force` uploads anyway and replaces what the store holds; the store keeps a snapshot of what it replaced, so `brain restore --from cloud` can undo it.

### A Self-Hosted Store

`brain cloud` also talks to a store you run yourself with [`brain-store`](/docs/advanced/self-hosting). Log in with the token its operator issued instead of the device code flow:

```bash
pbpaste | brain cloud login --api-url https://store.example.com --token-stdin
```

`--token TOKEN` and the `BRAIN_STORE_TOKEN` environment variable also work. The CLI refuses to send a token over plain HTTP to another host unless you pass `--allow-http`.
