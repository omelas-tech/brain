...
<Callout type="tip">
  [`/brain:memorize`](/docs/commands/memorize/) proactively proposes pinning when it stores a durable convention — so you often don't need to pin manually. `/brain:pin` is a toggle: run it with `--off` (or `unpin`) to remove a pin.
</Callout>
