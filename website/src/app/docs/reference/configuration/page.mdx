...
export const metadata = docMeta("/docs/reference/configuration", {
...
Brain configuration lives in `~/.brain/index.json` under the `config` key. All settings have sensible defaults set when the brain is created (at install, or on first sync).

## Environment Variables

| Variable | Default | Description |
|----------|---------|-------------|
| `BRAIN_DIR` | `~/.brain` | Location of the brain folder. Point it at a synced folder (Google Drive, Dropbox, iCloud Drive, OneDrive, Syncthing) or a git working copy to sync across devices with no account. A leading `~/` is expanded; honored everywhere — recall, memorize, init, and Cloud sync. |
...
| `auto_consolidate` | boolean | true | Whether to suggest consolidation when candidates are found during recall. Set to false to only consolidate via `/brain:sleep`. |
...
## Runtime Config (`~/.brain/config.json`)

A second, smaller file — `~/.brain/config.json`, created lazily — holds what the CLI and hooks read at runtime: token budgets, quarantine mode, prompt-time recall, and sensitive-topic consent. Missing keys fall back to these defaults:
...
| Setting | Type | Default | Description |
|---|---|:---:|---|
| `prompt_recall_top` | integer | 3 | Maximum memories the `UserPromptSubmit` hook injects alongside a prompt. `0` disables prompt-time recall. |
| `prompt_recall_budget_tokens` | integer | 600 | Token cap for that injection. The weakest matches are dropped first. |
| `sensitive_topics` | boolean | false | Opt in to storing `sensitive` memories (health, race, ethnicity, religious beliefs, politics, gender identity or sexual orientation). While `false`, such writes are quarantined as `sensitive_opt_out` and hidden until approved one by one with `brain verify approve`. Never retroactive. |

The four `*_budget_tokens` keys are described in [Working Memory](/docs/advanced/working-memory/#the-budget) and `quarantine_mode` in [Provenance & Trust](/docs/concepts/provenance-trust/#quarantine-untrusted-writes-wait-for-review).
