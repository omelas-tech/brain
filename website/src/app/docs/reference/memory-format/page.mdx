...
export const metadata = docMeta("/docs/reference/memory-format", {
...
| Field | Type | Required | Description |
|---|---|:---:|---|
| `sensitivity` | string | No | Consent tier. `standard` (default — omitted from the file), `sensitive` (health, race, ethnicity, religious beliefs, politics, gender identity or sexual orientation; stored only with consent), or `blocked` (government ID numbers, criminal history, immigration status; refused at write time, so never present on disk) |

Only non-`standard` values are written to frontmatter and the index. See [Sensitivity](/docs/commands/memorize/#sensitivity) for the write-time and recall behavior.
