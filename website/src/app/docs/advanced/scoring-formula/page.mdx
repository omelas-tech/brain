...
export const metadata = docMeta("/docs/advanced/scoring-formula", {
...
score = ( 0.38 * relevance
        ...
        + 0.14 * context_match
        + 0.08 * salience ) * trust_factor(origin)
```

All six factors are normalized to the range 0.0 - 1.0 before weighting. The
weighted composite is then multiplied by an [origin trust
factor](/docs/concepts/provenance-trust/) (0.75 - 1.0).
...
Activation spreads up to 2 hops from source memories, decaying by 50% per hop. The spreading bonus is capped at 1.0. Each source's activation strength is damped by its [origin trust factor](/docs/concepts/provenance-trust/), so a cluster of untrusted memories spreads less activation than an equally connected cluster of user-stated ones.
...
### Origin Trust (multiplier: 0.75 - 1.0)

After the six weighted factors are combined, the composite is multiplied by a trust factor derived from the memory's [origin](/docs/concepts/provenance-trust/):

| Origin | Trust factor |
|---|:---:|
| `user` | 1.00 |
| `agent-inferred` (and memories with no origin) | 0.95 |
| `tool-output` | 0.85 |
| `external` | 0.75 |

Unlike the weighted factors, trust is a multiplier: it bounds how far a memory's score can reach rather than contributing its own signal. A fact planted by a web page must be *substantially* more relevant than a user-stated fact to outrank it — it can never get there on volume or clique amplification alone.
