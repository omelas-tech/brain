...
export const metadata = docMeta("/docs/advanced/spaced-reinforcement", {
...
<Callout type="tip">
  The spaced repetition review system reinforces due memories at optimal intervals, maximizing the spacing effect. It runs automatically during the `/brain:sleep` cycle — no manual step needed.
</Callout>

<Callout type="info">
  Spaced reinforcement is applied automatically during any recall — whether through explicit `/brain:remember`, automatic session-start loading, or the `/brain:sleep` cycle. You do not need to do anything special to benefit from it.
</Callout>
