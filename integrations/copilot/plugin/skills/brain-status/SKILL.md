---
name: brain-status
description: Show a health dashboard of the user's brain memory system (~/.brain) — memory counts, effective strength, life-domain distribution, association network, review queue, and sync state. Use when the user asks about their brain's status or health, how many memories exist, or what the memory system knows overall.
license: MIT
---

# Brain Memory — Status Dashboard

Display an overview of the Brain Memory system's current state by reading its
JSON indexes directly (this is a read-only report — do not modify any files).
...
2. **Compute statistics.** For each memory:
   ...
   cognitive type, confidence distribution, count below the 0.3 consolidation
   threshold, review-queue due counts.
