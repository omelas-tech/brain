---
name: brain-memorize
description: Store durable memories (decisions, insights, learnings, preferences, goals, experiences) into the user's global brain (~/.brain) via the brain CLI. Use when the user says "remember this" or "memorize", or when a session produced notable decisions, learnings, or insights worth keeping across sessions and agents.
license: MIT
---

# Brain Memory — Memorize

Decide **what** to remember and **how to classify** it; the `brain memorize`
CLI handles all file operations (IDs, strength/decay computation, directories,
index updates, association edges, search index).

**Default: store immediately, report after.** When the user asked to
memorize, do not ask for confirmation. If they didn't specify content,
extract the most significant decisions, learnings, insights, or experiences
...
- **path**: life-domain hierarchy under `~/.brain` — `professional/`,
  ...
  `personal/health/sleep-routine.md`
...
```bash
BRAIN_AGENT=copilot-cli brain memorize <<'EOF'
{
  "memories": [
    {
...
- **Contradictions:** if the CLI reports `potential_conflicts` with a
  ...
  supersede the old fact, keep both scoped (e.g. per project), or reject the
  new one. Never silently keep both.
- **Never store secrets, credentials, or trivia.**
- If the `brain` command is not found, tell the user once: install with
